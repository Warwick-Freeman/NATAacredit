        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {[
            ...
            <div key={k} className="card card-pad" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: 8, background: item.on ? 'var(--good-soft)' : 'var(--surface-2)', color: item.on ? 'var(--good)' : 'var(--ink-3)', display: 'grid', placeItems: 'center' }}>
                <Icon name={item.on ? "check" : "link"} size={16} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 500 }}>{item.n}</div>
                <div style={{ fontSize: 11, color: 'var(--ink-3)' }}>{item.t}</div>
              </div>
              <Pill kind={item.on ? "good" : "outline"} dot>{item.on ? "Connected" : "Off"}</Pill>
            </div>
          ))}
        </div>
      )}
